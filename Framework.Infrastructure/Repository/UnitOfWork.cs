using System.Data;  // Provides access to database transaction isolation levels
using Framework.Abstractions.Repository;  // Includes the IUnitOfWork interface definition
    ...
    where TDbContext : DbContext
{
        ...
        CancellationToken cancellationToken = default)
    {
        if (commandLifetime is not null)
            context.Database.SetCommandTimeout((int)commandLifetime.Value.TotalSeconds);  // Sets command timeout if specified

        await context.Database.BeginTransactionAsync(isolationLevel, cancellationToken);  // Begins the transaction with the specified isolation level
    }
